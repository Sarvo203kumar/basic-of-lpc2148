...
void command(uint8_t cmd)
{
    ...
    IO1CLR = 0xF << d4;          // Clear data lines
    IO1SET = (cmd & 0xF0) << (d4 - 4); // Send higher nibble
    IO1SET = (1 << en);          // EN = 1
    delay_ms(1);
    IO1CLR = (1 << en); // EN = 0
    delay_ms(1);
...
void write(char data)
{
    ...
    IO1CLR = 0xF << d4;          // Clear data lines
    IO1SET = (data & 0xF0) << (d4 - 4); // Send higher nibble
    IO1SET = (1 << en);          // EN = 1
    delay_ms(1);
    IO1CLR = (1 << en); // EN = 0
    delay_ms(1);
...
int main()
{
    ...
    IO0DIR |= (1 << 19) | (1 << 20); // Set motor control pins as output
    init();
